...
export function useVaultContributors(vaultAddress?: Address) {
  ...
  const [backendWallets, setBackendWallets] = useState<Address[] | undefined>(
    undefined
  );
  ...
  const fetchBackendContributors = useCallback(async (vault?: Address) => {
    ...
    try {
      ...
      if (res.ok && json.success && Array.isArray(json.data)) {
        const walletsFromBackend = json.data.map(
          (c: any) => c.wallet as Address
        );
...
export function useContributorInfo(
...
// New hook to fetch full contributor data from backend (for nominate modal, etc)
export function useBackendContributors(vaultAddress?: Address) {
    ...
    const fetchContributors = async () => {
      ...
      try {
        ...
        const res = await fetch(
          `${apiBase}/contributors/vault/${vaultAddress}`
        );
        ...
        if (json.success && Array.isArray(json.data)) {
          // Add isActive = true by default for backend contributors
          const contributorsWithActive = json.data.map((c: any) => ({
            ...c,
            wallet: c.wallet as Address,
            isActive: c.isActive !== undefined ? c.isActive : true,
          }));
          setContributors(contributorsWithActive);
        } else {
          setContributors([]);
        }
      } catch (err) {
        console.error("Failed to fetch backend contributors:", err);
