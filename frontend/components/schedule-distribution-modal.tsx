...
import {
  useApiUpcomingDistributions,
  useApiRecentDistributions,
} from "@/hooks/use-api";
...
export function ScheduleDistributionModal({
  open,
  onOpenChange,
  vaultAddress,
  ...
  const { scheduleDistribution, isPending, isSuccess, hash } =
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          ...
          <DialogDescription>
            Set up an automated yield distribution to contributors
          </DialogDescription>
        </DialogHeader>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          {!vaultAddress && vaultsWithNames && vaultsWithNames.length > 0 && (
            <div>
              ...
              <Select
                ...
                onValueChange={(value) =>
                  setFormData({ ...formData, vault: value })
                }
              >
                <SelectTrigger>
          ...
          <div className="bg-muted/50 p-3 rounded-lg text-sm">
            ...
            <p className="text-foreground/70">
              {formData.vault && formData.date
                ? `Distribution on ${formData.date} to all contributors in ${
                    formData.vault || "selected vault"
                  }`
          ...
          <div className="flex gap-2 pt-4">
            ...
            <Button
              type="submit"
              disabled={
                isPending || !formData.vault || !formData.date || !formData.time
              }
