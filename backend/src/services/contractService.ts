...
export class ContractService {
  ...
  async getVaultInfo(vaultAddress: string) {
    ...
    try {
      const [name, description, totalAssets, totalSupply] =
      ...
      throw new Error(
        `Contract at ${vaultAddress} does not implement getVaultInfo() or is not a valid vault contract. Error: ${error.message}`
      );
    }
  }
  ...
  async getContributor(vaultAddress: string, contributorAddress: string) {
    ...
    const contributor = await registry.getContributor(
      vaultAddress,
      contributorAddress
    );
    return {
