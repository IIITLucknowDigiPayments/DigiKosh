...
export class ContributorController {
  ...
  async createContributor(req: Request, res: Response) {
    ...
    try {
      ...
      // Update vault stats: increment contributor count and add monthlyAllocation to totalAssets
      try {
        const vaultDoc = await Vault.findOne({ address: vault });
        if (vaultDoc) {
          // Increment contributor count
          vaultDoc.contributorCount = (vaultDoc.contributorCount || 0) + 1;

          // Add monthly allocation to totalAssets
          try {
            const currentAssets = BigInt(vaultDoc.totalAssets || "0");
            const allocationAmount = BigInt(monthlyAllocation || "0");
            const newTotalAssets = currentAssets + allocationAmount;
            vaultDoc.totalAssets = newTotalAssets.toString();
          } catch (e) {
            console.error("Error calculating totalAssets:", e);
          }
          ...
          try {
            ...
            vaultDoc.monthlyYield = yield07percent.toString();
          } catch (e) {
            console.error("Error calculating monthlyYield:", e);
          }

          await vaultDoc.save();
          console.log("Vault stats updated:", vaultDoc);
        }
      } catch (error: any) {
